...
# - HasEvo: The Pokémon has an evolution
# - HasPreevo: The Pokémon has a preevolution


# MORE INFO TO INCLUDE IN THE FUTURE:
# - Against(Type): Amount of damage taken for a specific attack type
...
def getPokemonData(name, inputUrl):   
    ...
    for elem in INFO:
        ...
        if elem == 'DexNumber':          # Get the National Pokedex ID of the Pokémon
            ...
            types_occurrencies = soup.find_all('a', href=lambda t: t and "(type)" in t)
            types = []
            j = 0
            ...
            category = soup.find('a', title=lambda t: t and "Pokémon category" in t).get_text()
            ...
            if  posregional not in REGIONAL_VARIATIONS:
                ...
                if not t.find('small', string=lambda t: t and "First Evolution" in t) is None:
                    ...
                    print(firstevo)
                secondevo = None 
                if not t.find('small', string=lambda t: t and "Second Evolution" in t) is None:
                    secondevo = t.find('small', string=lambda t: t and "Second Evolution" in t).find_next('span').get_text()
                    print(secondevo)
            else:
                fullform = posregional + ' Form'
                found = False
                realt = None
                ts = soup.find_all('small', string=lambda t: t and "Unevolved" in t)
                j = 0
                while not found:
                    t = ts[j].find_parent('table').find_parent('table')
                    unevolved = t.find('small', string=lambda t: t and "Unevolved" in t).find_next('span').get_text()
                    form = t.find('small', string=lambda t: t and "Unevolved" in t).find_next('span').find_next('small')
                    if unevolved == name and form == fullform:
